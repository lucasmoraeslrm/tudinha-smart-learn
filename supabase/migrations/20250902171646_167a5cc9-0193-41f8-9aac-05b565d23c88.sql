...
-- 3) RLS policies (drop existing and recreate)
-- 3a) Drop all existing chat policies
DO $$
DECLARE
  pol_name text;
BEGIN
  ...
  LOOP
    EXECUTE 'DROP POLICY "' || pol_name || '" ON public.chats';
  END LOOP;
END
$$;

-- 3b) Drop all existing message policies
DO $$
DECLARE
  pol_name text;
BEGIN
  ...
  LOOP
    EXECUTE 'DROP POLICY "' || pol_name || '" ON public.messages';
  END LOOP;
END
$$;

-- 4) Create new unified policies
...
-- 6) Trigger to keep chats.last_message_at updated on message inserts (uses existing function)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
