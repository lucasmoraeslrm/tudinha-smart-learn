  ...
  const fetchProfessors = async () => {
    ...
    try {
      ...
      if (ids.length > 0) {
        ...
        (rels || []).forEach((r: any) => {
          if (r.materia_id) {
            if (!matMap[r.professor_id]) matMap[r.professor_id] = [];
            matMap[r.professor_id].push(r.materia_id);
          }
          if (r.turma_id) {
            if (!turMap[r.professor_id]) turMap[r.professor_id] = [];
            turMap[r.professor_id].push(r.turma_id);
          }
        });
                     ...
                     {turmasOptions.map((turma) => (
                       <div key={turma.id} className="flex items-center space-x-2">
                         <Checkbox
                           ...
                           onCheckedChange={(checked) => 
                             handleTurmaToggle(turma.id, !!checked)
                           }
                         />
                         <Label 
                     ...
                     <TableCell>
                       <div className="flex flex-wrap gap-1">
                         {(profTurmasMap[professor.id] || []).map((turmaId) => (
                           <Badge key={turmaId} variant="secondary" className="text-xs">
                             {turmaNameById[turmaId] || turmaId}
                           </Badge>
                         )) || '-'}
                       </div>
                     </TableCell>
